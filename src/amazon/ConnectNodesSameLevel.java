...
public class ConnectNodesSameLevel {
    ...
    void connect(cNode p) {
        // initialize queue to hold cNodes at same level
        Queue<cNode> q = new LinkedList<>();

        q.add(root); // adding cNodes to tehe queue

        cNode temp = null; // initializing prev to null
        while (!q.isEmpty()) {
            int n = q.size();
            for (int i = 0; i < n; i++) {
                cNode prev = temp;
                ...
                // the last cNode of previous level,
                // so we skip it
                if (i > 0)
                    prev.nextRight = temp;
            ...
            // pointing last cNode of the nth level to null
            temp.nextRight = null;
        }
    }
    ...
    public static void main(String args[]) {
        ...
        */
        ...
        // Populates nextRight pointer in all cNodes
